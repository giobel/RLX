...
namespace RLX
{
    ...
    public class FamilyInstanceToDSPipeFitt : IExternalCommand
    {
                ...
                foreach (GeometryObject geoObj in geometryElement)
                {
                    ...
                    foreach (var item in ge)
                    {
                        ...
                        //{
                            try
                            {
                                DirectShape directShape = DirectShape.CreateElement(doc, new ElementId(BuiltInCategory.OST_PlaceHolderPipes));
                                directShape.SetShape(new List<GeometryObject>() { item });
                                counter++;
                            }
                            catch { }
